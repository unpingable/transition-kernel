...
| **ComposedReceiptSnapshotCoherence** | `Scratch/ExecutionRevalidation.lean` · SCRATCH (uncommitted) | (c) | the composed receipt must pin the execution snapshot (`revalidated_at` + `valid_until`, not just the eligibility reference) that governed the effect; two execution snapshots over one lineage must be distinguishable in the receipt | `composed_snapshot::ComposedExecutionSnapshot` + `chain::check_correspondence` + `agent_gov` `transition_enforce.{composed_snapshot_hash,verify_snapshot_coherence,reconstruct_composed}` + `specimens/receipt-bundle/*/reconstruct_composed.json` | **OPERATIONAL GATE SHIPPED (Stage 3c)** — `CORRESPONDENCE.md` → Stage 3c. The Lean scratch model is **upstream evidence** (Lean→kernel-design), not a stated theorem over the public API: `Execution.lean` emits no receipt, so the property is *unstatable* there. This deployed gate is the forcing consumer a downstream theorem would justify. **Not ratified:** a green Rust build is not a verified reduction (proof→world fence). |
| **NoFreeContinuation** (GAP-2) | `Scratch/NoFreeContinuation.lean` · SCRATCH (uncommitted, hand-managed lean repo) | (a)+(c) | a successful governed act does not authorize the next step: continuation authority is chain-tip bound, single-use (no contraction / no replay), and grants an *attempt* not a success — `EffectSucceeded ⊬ MayContinue` | `continuation::{decide_continuation, ContinuationGrant, ContinuationConsumer}` (C1) + `agent_gov` `continuation_enforce.{present_continuation, reconstruct_continuation}` (C3) + `specimens/continuation-trajectory/` (C4) | **OPERATIONAL GATE SHIPPED (GAP-2 C1–C4); Lean feedstock authored.** Models the C4 spine (tip-binding + success + single-use burn), schematic over scope/class/version; axiom-clean (propext at most). The deployed C3 enforce gate + C4 trajectory are the forcing consumer that makes the theorem worth stating. **Not ratified:** a green Lean build is upstream design evidence, not a verified reduction of the Rust gate (proof→world fence). Seed: *a successful governed act does not authorize the agent's next breath.* |
